...
class CameraConnectionServiceContractTest {
    ...
    fun foregroundAndNotificationUseExplicitNonStickyDisconnectWithoutCredentials() {
        ...
        assertTrue(service.contains(".setAction(ACTION_CANCEL_DOWNLOAD)"))
        val cancelAction = service.substringAfter("ACTION_CANCEL_DOWNLOAD -> {").substringBefore("ACTION_DISCONNECT ->")
        assertTrue(cancelAction.contains("cancelDownload()"))
        assertTrue(cancelAction.contains("Cancelar recibido desde notificación"))
        assertFalse(cancelAction.contains("disconnect()"))
        assertTrue(service.contains("fun cancelDownload() = downloader.cancel()"))
        assertTrue(service.contains("notificationUpdates.shouldPublish("))
